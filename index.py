    ...
    with psycopg2.connect(DATABASE_URL) as conn:
        with conn.cursor() as c:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS interactions (
                id SERIAL PRIMARY KEY, user_query TEXT, ai_response TEXT, timestamp TIMESTAMPTZ, 
            ...
            if c.fetchone()[0] == 0:
                c.execute("INSERT INTO users (username, password, role) VALUES (%s, %s, %s) ON CONFLICT (username) DO NOTHING", ('admin', hash_password('admin123'), 'admin'))
                c.execute("INSERT INTO users (username, password, role) VALUES (%s, %s, %s) ON CONFLICT (username) DO NOTHING", ('client', hash_password('client123'), 'client'))
            ...
            if c.fetchone()[0] == 0:
                ...
                c.executemany("INSERT INTO glossary (term, definition) VALUES (%s, %s) ON CONFLICT (term) DO NOTHING", sample_terms)
            conn.commit()
...
def get_document_content(doc_id):
    ...
    print(f"CACHE MISS: Document {doc_id} not found in cache. Fetching from GDrive.")
    ...
    try:
        ...
        DOCUMENT_CACHE[doc['id']] = {'content': content, 'display_name': doc['display_name'], 'uploaded_by': doc['uploaded_by']}
        return content
...
def get_smart_knowledge_base_content(query, client_id=None):
    ...
    for doc_id, doc_data in DOCUMENT_CACHE.items():
        ...
        if not (is_admin_doc or is_client_doc): continue
...
def login_required(role="ANY"):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            conn.close()
            if not user:
                session.clear(); flash('Your user account could not be found. Please log in again.', 'danger'); return redirect(url_for('login'))
            if role != "ANY" and user['role'] != role:
                flash('You do not have permission to access that page.', 'danger'); return redirect(url_for(f"{user['role']}_dashboard"))
...
def login():
    if 'user_id' in session:
        role = session.get('role', 'client'); return redirect(url_for(f'{role}_dashboard'))
...
@login_required()
def logout():
    session.clear(); flash('You have been logged out successfully.', 'info'); return redirect(url_for('login'))

@app.route('/lumon/signup')
def lumon_signup():
    settings = get_settings(); return render_template('severance_signup.html', settings=settings, show_widget=True)
...
def admin_dashboard():
    conn = get_db_connection()
    with conn.cursor() as c:
        c.execute('SELECT COUNT(*) FROM documents'); total_docs = c.fetchone()[0]
        c.execute('SELECT COUNT(*) FROM interactions WHERE DATE(timestamp) = CURRENT_DATE'); queries_today = c.fetchone()[0]
        c.execute('SELECT AVG(response_time_seconds) FROM interactions WHERE response_time_seconds IS NOT NULL'); avg_response_time = c.fetchone()[0] or 0
        c.execute('SELECT id, display_name, upload_timestamp FROM documents ORDER BY upload_timestamp DESC LIMIT 5'); recent_docs = c.fetchall()
        labels, data = [], []
        for i in range(6, -1, -1):
            day = datetime.now() - timedelta(days=i)
            labels.append(day.strftime('%a'))
            c.execute('SELECT COUNT(*) FROM interactions WHERE DATE(timestamp) = %s', (day.strftime('%Y-%m-%d'),)); data.append(c.fetchone()[0])
    conn.close()
...
def admin_documents():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn.commit()
            file_stream.seek(0)
            new_content = extract_text_from_pdf(file_stream) if filetype == 'pdf' else extract_text_from_docx(file_stream)
            DOCUMENT_CACHE[new_doc_id] = {'content': new_content, 'display_name': original_filename, 'uploaded_by': admin_id}
            print(f"CACHE ADD: Added document {new_doc_id} to cache.")
            flash('Document uploaded successfully!', 'success')
        except Exception as e:
            flash(f'Failed to upload document: {e}', 'danger'); traceback.print_exc()
        finally:
            conn.close()
        ...
        if doc:
            try:
                ...
                if doc_id in DOCUMENT_CACHE:
                    ...
                    print(f"CACHE REMOVE: Removed document {doc_id} from cache.")
...
def client_documents():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            file_stream.seek(0)
            new_content = extract_text_from_pdf(file_stream) if filetype == 'pdf' else extract_text_from_docx(file_stream)
            DOCUMENT_CACHE[new_doc_id] = {'content': new_content, 'display_name': original_filename, 'uploaded_by': client_id}
            print(f"CACHE ADD: Added document {new_doc_id} to cache.")
...
def admin_analytics():
    conn = get_db_connection()
    with conn.cursor() as c:
        c.execute('SELECT * FROM interactions ORDER BY timestamp DESC'); interactions = c.fetchall()
        c.execute('SELECT COUNT(*) FROM interactions WHERE feedback_score = 1'); liked_count = c.fetchone()[0]
        c.execute('SELECT COUNT(*) FROM interactions WHERE feedback_score = -1'); disliked_count = c.fetchone()[0]
        c.execute('SELECT COUNT(*) FROM interactions WHERE feedback_score IS NULL OR feedback_score = 0'); no_rating_count = c.fetchone()[0]
        feedback_counts = {'liked': liked_count, 'disliked': disliked_count, 'no_rating': no_rating_count}
        c.execute('SELECT AVG(response_time_seconds), MIN(response_time_seconds), MAX(response_time_seconds) FROM interactions WHERE response_time_seconds IS NOT NULL'); timing_data = c.fetchone()
        c.execute('SELECT user_query FROM interactions'); all_queries = c.fetchall()
    conn.close()
    words = []
...
def admin_settings():
        ...
        with conn.cursor() as c:
            if 'add_user' in request.form:
                ...
                if username and password and role:
                    try:
                        c.execute('INSERT INTO users (username, password, role) VALUES (%s, %s, %s)', (username, hash_password(password), role)); conn.commit()
                        flash('User added successfully.', 'success')
                    except psycopg2.IntegrityError:
                        conn.rollback(); flash('Username already exists.', 'danger')
                else:
    ...
    with conn.cursor() as c:
        c.execute('SELECT id, username, role FROM users'); users = c.fetchall()
        c.execute('''
        ...
        '''); api_keys = c.fetchall()
        c.execute('SELECT key, value FROM settings'); settings_data = c.fetchall()
    conn.close()
...
def admin_delete_user(user_id):
    if user_id == session.get('user_id'):
        flash('You cannot delete your own account.', 'danger'); return redirect(url_for('admin_settings'))
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute('DELETE FROM users WHERE id = %s', (user_id,)); conn.commit()
        flash('User and their associated data have been deleted.', 'success')
    except Exception as e:
        conn.rollback(); flash(f'Error deleting user: {e}', 'danger')
    finally:
        conn.close()
...
def admin_glossary():
    ...
    if request.method == 'POST':
        ...
        if term and definition:
            try:
                with conn.cursor() as c:
                    c.execute('INSERT INTO glossary (term, definition) VALUES (%s, %s)', (term, definition)); conn.commit()
                flash(f'Term "{term}" added successfully!', 'success')
            except psycopg2.IntegrityError:
                conn.rollback(); flash(f'Term "{term}" already exists.', 'danger')
        else:
            flash('Both term and definition are required.', 'warning')
        return redirect(url_for('admin_glossary'))
    with conn.cursor() as c:
        c.execute('SELECT * FROM glossary ORDER BY term'); glossary_terms = c.fetchall()
    conn.close()
...
def admin_delete_term(term_id):
    conn = get_db_connection()
    with conn.cursor() as c:
        c.execute('DELETE FROM glossary WHERE id = %s', (term_id,)); conn.commit()
    conn.close()
    flash('Term deleted successfully.', 'success'); return redirect(url_for('admin_glossary'))
...
def client_dashboard():
    ...
    conn = get_db_connection()
    with conn.cursor() as c:
        c.execute('SELECT COUNT(*) FROM interactions WHERE client_id = %s AND DATE(timestamp) = CURRENT_DATE', (client_id,)); queries_today = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM interactions WHERE client_id = %s AND DATE_TRUNC('month', timestamp) = DATE_TRUNC('month', CURRENT_DATE)", (client_id,)); queries_this_month = c.fetchone()[0]
        c.execute('SELECT COUNT(*) FROM api_keys WHERE client_id = %s', (client_id,)); api_key_count = c.fetchone()[0]
        labels, data = [], []
        for i in range(6, -1, -1):
            day = datetime.now() - timedelta(days=i)
            labels.append(day.strftime('%a'))
            c.execute('SELECT COUNT(*) FROM interactions WHERE client_id = %s AND DATE(timestamp) = %s', (client_id, day.strftime('%Y-%m-%d'))); data.append(c.fetchone()[0])
    conn.close()
...
def client_api_key():
    ...
    if request.method == 'POST':
        ...
        with conn.cursor() as c:
            c.execute('INSERT INTO api_keys (client_id, api_key, purpose, issuance_timestamp) VALUES (%s, %s, %s, %s)', (client_id, api_key, purpose, datetime.now()))
        conn.commit(); conn.close()
        flash(f'New API Key generated for "{purpose}"!', 'success'); return redirect(url_for('client_api_key'))
    with conn.cursor() as c:
        c.execute('SELECT * FROM api_keys WHERE client_id = %s ORDER BY issuance_timestamp DESC', (client_id,)); api_keys = c.fetchall()
...
def client_delete_api_key(key_id):
    ...
    conn = get_db_connection()
    with conn.cursor() as c:
        c.execute('SELECT * FROM api_keys WHERE id = %s AND client_id = %s', (key_id, client_id)); key = c.fetchone()
        if key:
            c.execute('DELETE FROM api_keys WHERE id = %s', (key_id,)); conn.commit()
...
def handle_chat():
    ...
    if api_key:
        conn = get_db_connection()
        with conn.cursor() as c:
            c.execute('SELECT client_id FROM api_keys WHERE api_key = %s', (api_key,)); client_row = c.fetchone()
        if client_row:
...
- Do NOT mention that you are an AI or refer to a knowledge cutoff date."""
        ...
        with conn.cursor() as c:
            c.execute('SELECT term, definition FROM glossary'); glossary_terms = c.fetchall()
        conn.close()
...
def handle_feedback():
    try:
        ...
        with conn.cursor() as c:
            ...
            if c.rowcount == 0:
                conn.close(); return jsonify({'error': 'Invalid response ID.'}), 404
        conn.commit(); conn.close()
